...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: diagnostic } = await supabase
    .from("diagnostics")
    .select(
      "id, title, framework, areas, additional_context, detail_level, tenant_id, status, clients(id, name, industry, strategic_context)"
    )
    .eq("id", diagnosticId)
    .single();
  ...
  try {
    ...
    return NextResponse.json({ ok: true, findingsCount: object.findings.length });
  } catch (err) {
